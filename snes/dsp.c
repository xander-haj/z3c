/*
 * dsp.c — Sony S-DSP audio synthesis implementation.
 *
 * Processes 8 voices at 32040 Hz (one sample per 32 APU cycles). Each voice:
 *   1. Decodes BRR-compressed samples (4-bit ADPCM, 16 samples per 9-byte block)
 *   2. Interpolates between samples using a 4-point Gaussian filter
 *   3. Applies pitch (with optional modulation from the previous voice)
 *   4. Shapes amplitude via ADSR envelope or manual gain control
 *
 * Mixed output passes through an echo unit: a ring buffer in APU RAM with an
 * 8-tap FIR filter, providing configurable reverb/delay effects.
 *
 * MY_CHANGES flag controls whether key-on/key-off is processed immediately
 * on register write (1) or deferred to the even-cycle phase (0).
 */
...
// Envelope rate lookup table. Index 0 = disabled (infinite), index 31 = fastest.
// Values represent the number of DSP cycles between envelope steps.
...
// Gaussian interpolation coefficient table (512 entries).
// Used for 4-point interpolation between decoded BRR samples.
// The table is symmetric: gaussValues[0xFF-x] and gaussValues[0x1FF-x] are
// the outer lobes, gaussValues[0x100+x] and gaussValues[x] are the inner lobes.
...
// Forward declarations for internal DSP processing functions.
static void dsp_cycleChannel(Dsp* dsp, int ch);      // Process one voice for one sample
static void dsp_handleEcho(Dsp* dsp, int* outputL, int* outputR); // Echo/FIR processing
static void dsp_handleGain(Dsp* dsp, int ch);         // Envelope state machine step
static void dsp_decodeBrr(Dsp* dsp, int ch);          // Decode next BRR block
static int16_t dsp_getSample(Dsp* dsp, int ch, int sampleNum, int offset); // Gaussian interp
static void dsp_handleNoise(Dsp* dsp);                // Advance noise LFSR
...
// Main DSP cycle — called once per sample (every 32 APU cycles = 32040 Hz).
// Pipeline: process all 8 voices → mix with volume → apply master volume →
// echo/FIR processing → noise generator → write stereo sample to output buffer.
void dsp_cycle(Dsp* dsp) {
  int totalL = 0;
  int totalR = 0;
  // Mix all 8 voices: each voice's sample is scaled by its per-channel volume.
  for(int i = 0; i < 8; i++) {
  ...
  totalL = totalL < -0x8000 ? -0x8000 : (totalL > 0x7fff ? 0x7fff : totalL);
  totalR = totalR < -0x8000 ? -0x8000 : (totalR > 0x7fff ? 0x7fff : totalR);
  // Process echo: reads from echo buffer, applies FIR filter, mixes with output,
  // writes new echo data back (voice mix + feedback) if echo writes are enabled.
  ...
  dsp_handleNoise(dsp);
  // Write the final stereo sample to the output ring buffer.
...
// Echo / reverb processing. Steps:
//   1. Read stereo sample from echo ring buffer in APU RAM
//   2. Feed through 8-tap FIR filter (circular buffer, oldest→newest)
//   3. Add filtered echo to main output (scaled by echo volume)
//   4. Compute new echo input: sum of echo-enabled voices + feedback
//   5. Write new echo sample back to APU RAM (if echo writes are enabled)
//   6. Advance ring buffer indexes; wrap when delay period expires
...
// Process one voice for one sample cycle. Steps:
//   1. Apply pitch (with optional modulation from previous voice's output)
//   2. Advance pitch counter; decode next BRR block if counter overflows
//   3. Interpolate sample using Gaussian filter (or use noise if enabled)
//   4. Handle key-on/key-off events (deferred to even cycles if !MY_CHANGES)
//   5. Apply envelope (ADSR or gain mode) to shape amplitude
//   6. Write envelope and sample values to DSP register mirror
...
// Envelope state machine — advances the gain value for one voice.
// States: 0=attack (ramp up), 1=decay (exp down to sustain level),
//         2=sustain (exp decrease), 3=gain (manual mode), 4=release (linear down).
// Attack adds 32 per step (or 1024 at fastest rate). Decay/sustain use exponential
// decrease: subtract (gain-1)/256 + 1. Gain mode supports 4 shapes.
...
// 4-point Gaussian interpolation between decoded BRR samples.
// `sampleNum` = integer sample index in decodeBuffer (0-15).
// `offset` = fractional position (0-255) between sampleNum and sampleNum+1.
// Uses the symmetric gaussValues[] table for the 4 filter taps.
...
// Decode one BRR (Bit Rate Reduction) block — the SNES's 4-bit ADPCM format.
// Each 9-byte block produces 16 PCM samples: 1 header byte + 8 data bytes
// (2 nybbles each). The header specifies shift amount (0-12) and filter (0-3).
// Filters use 1 or 2 previous samples for prediction (IIR-style).
// The last 3 decoded samples are preserved at the start of decodeBuffer
...
// Advance the global noise generator (15-bit LFSR).
// Taps bits 0 and 1, XORs them into bit 14. Rate controlled by FLG register.
// Voices with useNoise=true substitute this output for their BRR sample.
...
// Write to a DSP register. Decodes the register address to update the
// appropriate voice or global state. Register layout: addresses $x0-$x9
// are per-voice (x = voice number), global registers are at fixed addresses.
// All writes are also mirrored to the register RAM for read-back.
void dsp_write(Dsp *dsp, uint8_t adr, uint8_t val) {
  int ch = adr >> 4; // Voice number (for per-voice registers)
  switch (adr) {
  case V0VOLL:
  case V1VOLL:
  case V2VOLL:
...
// Copy the DSP's internal sample buffer to the caller's output buffer.
// Resamples from the native 534 samples/frame to the requested count using
// nearest-neighbor interpolation. Supports mono (channels averaged) or stereo.
