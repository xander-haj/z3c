/*
 * dma.c — SNES DMA / HDMA controller implementation.
 *
 * General DMA: Bulk transfers between A-bus (CPU address space) and B-bus
 * (PPU/APU registers). Transfers block the CPU and run at 8 master cycles
 * per byte. Each channel has configurable source, destination, direction,
 * stepping mode, and transfer pattern (mode 0-7).
 *
 * HDMA: Table-driven per-scanline transfers during H-blank. Each channel
 * reads a repeat count + data values from a table in ROM/RAM. Supports
 * indirect mode where the table contains pointers to the actual data.
 * Used for scroll effects, color gradients, HDMA windowing, etc.
 */
...
// B-bus address offset patterns for each transfer mode (0-7).
// Each mode defines which PPU register offsets are written per transfer unit.
// E.g., mode 1 alternates between bAdr+0 and bAdr+1 (for VRAM low/high).
...
// Number of bytes per transfer unit for each DMA mode.
...
// Reset all 8 channels to power-on defaults. Per hardware spec, most fields
// are initialized to 0xFF/true (all bits set), matching open-bus behavior.
...
// Read a DMA channel register ($43x0-$43xF). The upper nibble of the address
// selects the channel (0-7), the lower nibble selects the register.
uint8_t dma_read(Dma* dma, uint16_t adr) {
  uint8_t c = (adr & 0x70) >> 4; // Channel number from address bits 4-6
  switch(adr & 0xf) {
    case 0x0: {
...
// Write a DMA channel register ($43x0-$43xF). Configures transfer parameters.
...
// Execute one byte of general DMA for the lowest-numbered active channel.
// Transfers one byte between A-bus and B-bus, advances the A-bus address
// (unless fixed mode), and decrements the byte counter. When a channel's
// counter reaches 0, it is deactivated and overhead cycles are added.
void dma_doDma(Dma* dma) {
  // Find the first active channel (lowest priority number wins).
  int i = 0;
  for(i = 0; i < 8; i++) {
...
// Initialize HDMA at frame start (scanline 0). For each active HDMA channel:
// loads the table address from aAdr, reads the first repeat count, and fetches
// the indirect address if indirect mode is enabled. Also cancels any pending
// general DMA on the same channel.
...
// Execute one scanline of HDMA for all active channels. For each non-terminated
// channel: if doTransfer is set, transfer `transferLength[mode]` bytes to/from
// the B-bus. Then decrement repCount; when the 7-bit counter portion reaches 0,
// read the next table entry (and new indirect address if applicable).
// Bit 7 of repCount controls whether the next line should transfer data.
...
// Transfer a single byte between A-bus (full 24-bit address) and B-bus (PPU register).
// Direction is controlled by `fromB`: true = B→A (PPU to CPU), false = A→B (CPU to PPU).
static void dma_transferByte(Dma* dma, uint16_t aAdr, uint8_t aBank, uint8_t bAdr, bool fromB) {
  // TODO: invalid writes:
...
// Consume one master cycle of DMA/HDMA time. HDMA timer takes priority
// (it runs during H-blank). Returns true if DMA is still busy (CPU halted).
...
// Enable DMA or HDMA channels based on the bitmask `val`.
// For general DMA ($420B), also sets dmaBusy and adds startup overhead.
// For HDMA ($420C), just marks channels as active for the next frame.
