/*
 * cpu.c — 65C816 CPU emulator implementation.
 *
 * Emulates the full 65C816 instruction set (~256 opcodes, many with 8/16-bit
 * variants). The 65C816 is a 16-bit extension of the MOS 6502, adding:
 *   - Switchable 8/16-bit accumulator and index registers (M and X flags)
 *   - 24-bit addressing via bank registers (program bank K, data bank DB)
 *   - 16-bit direct page register (DP) — relocatable zero page
 *   - Native mode (16-bit) and emulation mode (6502-compatible)
 *   - Block move instructions (MVN/MVP)
 *   - Long addressing modes (absolute long, indirect long)
 *
 * Interrupt handling: NMI takes priority over IRQ. Both push K, PC, and
 * flags to the stack and vector through addresses in bank $00 ($FFEA for
 * NMI, $FFEE for IRQ in native mode).
 *
 * The opcode dispatch is a flat switch statement. Addressing mode helpers
 * return both the low and high byte addresses for 16-bit access. ALU
 * operations respect the M/X flags for operand width.
 */
...
// Base cycle count for each 65C816 opcode. Additional cycles may be added
// for 16-bit operations, page crossings, direct page misalignment, etc.
...
// Forward declarations — core CPU operations.
static uint8_t cpu_read(Cpu* cpu, uint32_t adr);           // Read via SNES bus (24-bit address)
static void cpu_write(Cpu* cpu, uint32_t adr, uint8_t val); // Write via SNES bus
static uint8_t cpu_readOpcode(Cpu* cpu);                   // Fetch byte at K:PC, advance PC
static uint16_t cpu_readOpcodeWord(Cpu* cpu);              // Fetch 16-bit word at K:PC
void cpu_setFlags(Cpu* cpu, uint8_t value);                // Unpack P register → individual flags
static void cpu_setZN(Cpu* cpu, uint16_t value, bool byte); // Set Z/N from result (8 or 16 bit)
static void cpu_doBranch(Cpu* cpu, uint8_t value, bool check); // Conditional relative branch
static uint8_t cpu_pullByte(Cpu* cpu);                     // Pop byte from stack
static void cpu_pushByte(Cpu* cpu, uint8_t value);         // Push byte to stack
static uint16_t cpu_pullWord(Cpu* cpu);                    // Pop 16-bit word from stack
static void cpu_pushWord(Cpu* cpu, uint16_t value);        // Push 16-bit word to stack
static uint16_t cpu_readWord(Cpu* cpu, uint32_t adrl, uint32_t adrh); // Read 16-bit from split addrs
static void cpu_writeWord(Cpu* cpu, uint32_t adrl, uint32_t adrh, uint16_t value, bool reversed);
static void cpu_doInterrupt(Cpu* cpu, bool irq);           // Push state, vector to NMI/IRQ handler
static void cpu_doOpcode(Cpu* cpu, uint8_t opcode);        // Main opcode dispatch

// Addressing modes and opcode helpers defined below (used only after definitions).

// Memory access — delegates to the SNES system bus. The cpu->mem pointer is
// a Snes* (typed as void* to avoid circular header dependencies).
...
// Power-on/reset: enter emulation mode, read reset vector from $FFFC/$FFFD.
// Emulation mode forces M=1, X=1 (8-bit registers) and constrains SP to page $01.
...
// Execute one CPU instruction or service a pending interrupt.
// Priority: STOP > WAI (wait for interrupt) > NMI > IRQ > normal opcode.
// Returns the number of master cycles consumed.
int cpu_runOpcode(Cpu* cpu) {
  cpu->cyclesUsed = 0;
  if(cpu->stopped) return 1;   // STP instruction: CPU halted until reset
  if(cpu->waiting) {            // WAI instruction: idle until interrupt arrives
...
// Pack individual flag booleans into the 8-bit P (processor status) register.
// Bit layout: N V M X D I Z C (bits 7-0).
...
// Unpack an 8-bit P value into individual flag booleans.
// In emulation mode (e=1): forces M=1, X=1, and constrains SP to page $01.
// When X flag is set: truncates X and Y registers to 8 bits.
...
// Service an interrupt: push K, PC, P to stack, disable IRQ, clear decimal,
// set K=0, and load PC from the appropriate vector.
// NMI vector: $FFEA/$FFEB. IRQ vector: $FFEE/$FFEF (native mode).
...
// Addressing modes
// ============================================================================
// Each mode returns the "high" address and stores the "low" address in *low.
// For 8-bit operations (M=1 or X=1), only *low is used.
// For 16-bit operations, both *low and the return value are read/written.
// The dp (direct page) modes add 1 cycle when DP is not page-aligned.

// Immediate: operand follows the opcode. 1 or 2 bytes depending on M/X flags.
...
// ============================================================================
// Main opcode dispatch — all 256 65C816 opcodes.
// ============================================================================
// Many opcodes have 8-bit and 16-bit variants controlled by the M flag
// (accumulator/memory width) or X flag (index register width). The `byte`
// variable is set from the relevant flag to select the correct operand size.
// RESTART label allows re-entry after breakpoint handling (spBreakpoint).
