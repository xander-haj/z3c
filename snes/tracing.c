/*
 * tracing.c — CPU and SPC700 instruction disassembly/tracing.
 *
 * Provides human-readable trace output for debugging. Each trace line shows:
 *   - Current address (bank:PC for CPU, or PC for SPC700)
 *   - Disassembled instruction with resolved operands
 *   - Full register state (A, X, Y, SP, DP/DB, flags)
 *
 * The disassembler uses format string tables indexed by opcode. Operand values
 * are read from memory at the current PC. The 65C816's variable-width immediates
 * (8 or 16 bit depending on M/X flags) are handled via a secondary format table.
 */
...
// 65C816 opcode name format strings. Each contains printf-style format specifiers
// for the operand bytes. 16-bit immediates are shown as %04x, 8-bit as %02x, etc.
...
static void getDisassemblyCpu(Snes* snes, char* line);  // Disassemble one 65C816 instruction
static void getDisassemblySpc(Apu *apu, char* line);   // Disassemble one SPC700 instruction

// Format a full 65C816 trace line: address, disassembly, and all registers/flags.
// Output format: "CPU BB:AAAA instruction A:XXXX X:XXXX Y:XXXX SP:XXXX DP:XXXX DB:BB e NVMXDIZC"
...
// Format a full SPC700 trace line: address, disassembly, and all registers/flags.
// Output format: "SPC AAAA instruction A:XX X:XX Y:XX SP:XX nvpbhizc"
