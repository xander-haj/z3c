/*
 * snes_other.c — ROM loading and cartridge header detection.
 *
 * Handles loading a ROM image into the emulator. The SNES ROM format has no
 * single standard header location — the internal header can appear at different
 * offsets depending on the mapping type and whether a 512-byte copier header
 * is prepended. This file reads all possible header locations, scores them for
 * plausibility (valid reset vector, reasonable field values, checksum match),
 * and selects the best candidate.
 *
 * After detection, the ROM is expanded to a power-of-2 size (mirroring the
 * tail to fill gaps), loaded into the cartridge, and the system is hard-reset.
 */
...
// Internal ROM header — parsed from the fixed location within the ROM image.
// The SNES internal header sits at $FFC0 (HiROM) or $7FC0 (LoROM) relative
// to the start of the ROM data (after any copier header is stripped).
...
// Load a ROM image: detect mapping type, strip copier header if present,
// expand to power-of-2 size, and load into the cartridge subsystem.
// Returns false if the ROM is too small or has an unsupported mapping type.
...
// Parse the internal ROM header at the given byte offset. Reads the game name,
// mapping speed, chip info, ROM/RAM sizes, region, and version. Computes a
// plausibility score based on field validity, checksum match, and whether the
// reset vector points to a reasonable opcode (SEI, CLC, JMP, etc.).
static void readHeader(uint8_t* data, int location, CartHeader* header) {
