/*
 * spc.c — SPC700 CPU emulator implementation.
 *
 * Emulates the full SPC700 instruction set (~256 opcodes) used by the SNES
 * audio subsystem. The SPC700 is an 8-bit processor with a 6502-like register
 * set (A, X, Y, SP, PC) plus a "direct page" flag (P) that selects between
 * two 256-byte pages for short addressing modes.
 *
 * Key differences from 6502:
 *   - 16-bit word moves (MOVW) and arithmetic (ADDW, SUBW, CMPW)
 *   - Bit manipulation: SET1/CLR1 (set/clear single bits), BBS/BBC (branch on bit)
 *   - Direct-page to direct-page operations (dp,dp addressing)
 *   - Memory-to-memory ALU (no need to route through accumulator)
 *   - MUL (Y*A→YA) and DIV (YA/X→A rem Y)
 *   - TCALL: 16 software interrupt vectors in the top of address space
 *
 * The opcode dispatch is a single switch statement. Helper functions handle
 * addressing modes (dp, abs, idx, idy, etc.) and ALU operations (adc, sbc,
 * and, or, eor, cmp, shifts, etc.) in both register and memory-to-memory forms.
 */
...
// Cycle count for each SPC700 opcode (indexed by opcode byte).
// Conditional branches add 2 cycles when taken.
...
static uint8_t spc_read(Spc* spc, uint16_t adr);         // Read byte from APU address space
static void spc_write(Spc* spc, uint16_t adr, uint8_t val); // Write byte to APU address space
static uint8_t spc_readOpcode(Spc* spc);                  // Fetch next byte at PC and advance
static uint16_t spc_readOpcodeWord(Spc* spc);             // Fetch next 16-bit word at PC
static uint8_t spc_getFlags(Spc* spc);                    // Pack flag bools → 8-bit PSW
static void spc_setFlags(Spc* spc, uint8_t value);        // Unpack 8-bit PSW → flag bools
static void spc_setZN(Spc* spc, uint8_t value);           // Set Z and N flags from result
static void spc_doBranch(Spc* spc, uint8_t value, bool check); // Conditional relative branch
static uint8_t spc_pullByte(Spc* spc);                    // Pop byte from stack (page $01)
static void spc_pushByte(Spc* spc, uint8_t value);        // Push byte to stack (page $01)
static uint16_t spc_pullWord(Spc* spc);                   // Pop 16-bit word from stack
static void spc_pushWord(Spc* spc, uint16_t value);       // Push 16-bit word to stack
static uint16_t spc_readWord(Spc* spc, uint16_t adrl, uint16_t adrh); // Read 16-bit from split addrs
static void spc_writeWord(Spc* spc, uint16_t adrl, uint16_t adrh, uint16_t value);
static void spc_doOpcode(Spc* spc, uint8_t opcode);       // Main opcode dispatch

// Addressing modes and opcode functions are defined below (used only after definitions).

// Memory access — delegates to APU which handles I/O port decode, DSP, timers, ROM overlay.
...
static void spc_write(Spc* spc, uint16_t adr, uint8_t val) {
  if (0 && adr == 0x5e)  // Debug hook (disabled)
...
// Reset all registers to 0, read the reset vector from $FFFE/$FFFF into PC.
...
// A stopped CPU returns 1 cycle (idle) until reset clears the stopped state.
...
// Addressing modes
// ============================================================================
// The SPC700's direct page (dp) is either page $00 or $01 based on the P flag.
// This is similar to the 6502's zero page but relocatable.

// Direct page: 8-bit operand + P flag selects page
...
//   - Register form (e.g., spc_and): operates on accumulator A
//   - Memory form (e.g., spc_andm): operates on memory (dst = dst OP src)
// SBC/CMP invert the operand and use ADC logic (value ^ 0xFF + carry).
...
// ============================================================================
// Main opcode dispatch — all 256 SPC700 opcodes.
// ============================================================================
// Opcode comments show: mnemonic, addressing mode.
// Grouped by opcode number. The SPC700 instruction encoding is irregular
// (unlike the 65C816's more systematic matrix), so this is a flat switch.
