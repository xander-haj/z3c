/*
 * sprite_main.h - Individual Enemy, NPC, and Boss Handler Declarations
 *
 * This header declares the handler functions for every sprite type in
 * The Legend of Zelda: A Link to the Past. Each enemy, NPC, boss, item,
 * and environmental entity has a dedicated update function (and often
 * a drawing function) that is dispatched by the sprite slot's type index
 * via SpriteActive_Main().
 *
 * The file is organized into logical groups:
 *   - Garnish allocation (particle effect slot management)
 *   - Boss handlers (Moldorm, Armos Knights, Lanmolas, Helmasaur King,
 *     Arrghus, Mothula, Blind, Kholdstare, Vitreous, Trinexx, Agahnim,
 *     Ganon, and their sub-sprites)
 *   - Soldier/guard variants (patrol, chase, weapon types)
 *   - Overworld enemies (Octoroks, Lynels, Moblins, Hinox, Leevers, etc.)
 *   - Dungeon enemies (Stalfos, Gibdo, Wizzrobe, Pirogusu, Wallmaster, etc.)
 *   - NPCs (Priest, Uncle, Zelda, Sahasrahla, Smithy, shopkeepers, etc.)
 *   - Collectibles and items (hearts, rupees, keys, fairies, bottles)
 *   - Environmental sprites (pull switches, statues, conveyor pipes)
 *   - SpritePrep_ initializers (run once when a sprite is first loaded)
 *
 * The parameter 'int k' in all handlers is the sprite slot index (0-15).
 * Drawing functions render the sprite into OAM using the sprite framework
 * from sprite.h.
 *
 * Depends on: types.h, sprite.h (PrepOamCoordsRet, SpriteHitBox, etc.)
 * Used by: sprite_main.c (contains all implementations)
 */
#pragma once
#include "types.h"
#include "sprite.h"
...
// ============================================================================
// Miscellaneous Utility and Bounce Entry Points
// ============================================================================

// Pull switch handler (bounce entry from SpriteActive_Main dispatch)
void Sprite_PullSwitch_bounce(int k);
// Giant Moldorm boss: draw body segments using position history lookback
void GiantMoldorm_DrawSegment_AB(int k, int lookback);
void GiantMoldorm_DrawSegment_C_OrTail(int k, int lookback);
// Cucco (chicken): increment subtype counter for attack escalation
void Chicken_IncrSubtype2(int k, int j);
// Scan for an Octoballoon parent sprite (for baby balloon linking)
bool Octoballoon_Find();
// Check if Link is within earshot of the flute boy
bool FluteBoy_CheckIfPlayerClose(int k);
// Fortune teller: display light world or dark world fortune text
void FortuneTeller_LightOrDarkWorld(int k, bool dark_world);

// ============================================================================
// Garnish Slot Allocation
// ============================================================================
// Garnishes (particle effects) occupy a pool of slots. These allocators
// find free slots using different strategies: force-overwrite, search from
// high or low indices, or limit to a maximum count per sprite.

int GarnishAllocForce();
int GarnishAlloc();              // Search high slots for a free garnish entry
int GarnishAllocLow();           // Search low slots for a free garnish entry
int GarnishAllocLimit(int k);    // Allocate with per-sprite count limit
int GarnishAllocOverwriteOldLow();  // Overwrite oldest low-slot garnish
int GarnishAllocOverwriteOld();     // Overwrite oldest any-slot garnish
...
// Wish Pond: handles item upgrade after throwing into the fairy fountain
void Sprite_WishPond3(int k);
// Spawn a small water splash particle at sprite k's position
int Sprite_SpawnSmallSplash(int k);
// Check if this heart piece/container was already collected (skip if so)
void HeartUpgrade_CheckIfAlreadyObtained(int k);
// Movable sanctuary mantle (pushed aside during rescue cutscene)
void Sprite_EE_MovableMantle(int k);
void Sprite_GoodOrBadArcheryTarget(int k);

// ============================================================================
// Soldier and Trooper Variants
// ============================================================================
// Hyrule Castle and overworld guards. Each variant has different weapons,
// patrol patterns, and aggression levels. They share common AI via
// Guard_Main() but have distinct drawing routines for their equipment.

void ChainBallTrooper_Draw(int k);
void Sprite_6B_CannonTrooper(int k);

// Bee: check if Link has an empty bottle to capture it
void Bee_PutInBottle(int k);
// Wizzrobe's beam projectile (homing magical attack)
void Sprite_Wizzbeam(int k);
// Kiki the monkey: initial idle state before Link approaches
void Kiki_LyingInwait(int k);
// Chain Chomp: fixed-point multiplication for chain link positioning
int ChainChomp_OneMult(uint8 a, uint8 b);
// Trinexx sub-sprites: fire head (CC) and ice head (CD)
void Sprite_CC(int k);
void Sprite_CD(int k);
// Initialization helper: offset sprite position by low 8 bits of coords
void SpritePrep_IncrXYLow8(int k);
void SpritePrep_FakeSword(int k);
void SpritePrep_MedallionTable(int k);
void Hobo_Draw(int k);
// Landmine: check if the Magic Hammer impact detonates this mine
...
// King Helmasaur's fireball projectile (splits into smaller fireballs)
void Sprite_70_KingHelmasaurFireball(int k);
// ============================================================================
// Wall Cannons and Mini-Games
// ============================================================================
...
// ============================================================================
// Dungeon Enemies: Debirando, Master Sword, Rollers, Beamos
// ============================================================================

// Debirando: fire-spitting pit enemy that emerges from floor holes
void Sprite_63_DebirandoPit(int k);
void DebirandoPit_Draw(int k);
void Sprite_64_Debirando(int k);
void Debirando_Draw(int k);
// Master Sword pedestal: cutscene sprite with light beam effects
...
// Spike roller: cylindrical trap that rolls back and forth
void Sprite_5D_Roller_VerticalDownFirst(int k);
void SpikeRoller_Draw(int k);
// Beamos: rotating laser eye turret that fires when it spots Link
...
// ============================================================================
// Overworld Ambient and Environmental Sprites
// ============================================================================

// Spark: electrical ball that travels along walls in a fixed direction
void Sprite_5B_Spark_Clockwise(int k);
// Decorative animals in the Lost Woods (no combat interaction)
void Sprite_59_LostWoodsBird(int k);
void Sprite_5A_LostWoodsSquirrel(int k);
// Crab: beach enemy that scuttles sideways
void Sprite_58_Crab(int k);
void Crab_Draw(int k);
// Desert Palace barrier statue: blocks passage until Book of Mudora is used
